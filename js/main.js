...
const iconMenu = document.querySelector(".icon-menu"); // Получаем в константу класс кнопки
...
if (menuLinks.length > 0) {
  ...
  function onMenuLinkClick(e) {
    ...
    if (
      ...
      /* Проверяем заполнен ли дата атрибут "data-goto" && 
		Проверяем, существует ли объект, на который ссылается этот артибут*/
      const gotoBlock = document.querySelector(menuLink.dataset.goto); // Получаем объект, на который ссылается атрибут "data-goto"
      const gotoBlockValue =
        gotoBlock.getBoundingClientRect().top + pageYOffset; // Высчитываем положение этого объекта
...
function videoMetaData() {
  let minutesLeft = Math.floor(
    (video.duration - Math.floor(video.currentTime)) / 60
  );
  if (minutesLeft < 10) {
...
const audioProgress = document.querySelector(
  ".traks .controls__progress input"
);
...
function audioMetaData() {
  let minutesLeft = Math.floor(
    (audio.duration - Math.floor(audio.currentTime)) / 60
  );
  if (minutesLeft < 10) {
    minutesLeft = "0" + String(minutesLeft);
  }
